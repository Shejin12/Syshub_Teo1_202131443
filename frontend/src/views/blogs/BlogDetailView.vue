...
const fetchBlog = async () => {
  try {
    const res = await axios.get(`http://localhost:3000/blogs/${route.params.id}`);
    blog.value = res.data;
  } catch (error) {
    console.error(error);
  } finally {
...
const submitComment = async () => {
  try {
    const url = replyingTo.value
      ? `http://localhost:3000/blogs/${blog.value.id}/comments/${replyingTo.value}/reply`
      : `http://localhost:3000/blogs/${blog.value.id}/comments`;
    ...
    fetchBlog(); // Refresh to show new comments
  } catch (error) {
    console.error(error);
  }
};
...
<template>
  ...
  <div v-else-if="blog" class="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
    <div class="bg-white rounded-2xl p-8 md:p-12 shadow-sm border border-slate-100">
      <h1 class="text-4xl md:text-5xl font-black text-slate-900 mb-6 leading-tight">{{ blog.title }}</h1>
      
      <div class="flex items-center gap-4 text-slate-500 text-sm mb-12 pb-8 border-b border-slate-100 justify-between">
        <div class="flex items-center gap-4">
          <RouterLink :to="`/users/${blog.author.id}`" class="flex items-center gap-3 hover:text-indigo-600 transition group">
            <div class="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center font-bold text-slate-600 group-hover:bg-indigo-100 group-hover:text-indigo-700 transition">
              {{ blog.author?.name?.charAt(0) }}
            </div>
            <div>
              <div class="font-bold text-slate-800 group-hover:text-indigo-600 text-base">{{ blog.author?.name }}</div>
              <div class="text-xs">{{ blog.author?.role?.name || blog.author?.role }}</div>
            </div>
          </RouterLink>
          <span class="text-slate-300">|</span>
          <span>Publicado el {{ new Date(blog.createdAt).toLocaleDateString() }}</span>
        </div>
        <button v-if="authStore.token" @click="openReport('BLOG', blog.id)" class="text-xs text-red-500 hover:text-red-700 font-medium px-2 py-1 bg-red-50 rounded hover:bg-red-100 transition">
          Reportar Artículo
        </button>
      </div>

      <div class="prose prose-indigo prose-lg max-w-none text-slate-800 markdown-body" v-html="compiledMarkdown"></div>
    </div>

    <!-- Seccion de Comentarios -->
    <div class="bg-slate-50 rounded-2xl p-8 border border-slate-200 mt-8">
      <h3 class="text-xl font-bold text-slate-800 mb-6">Comentarios ({{ blog.comments?.length || 0 }})</h3>
      
      <!-- Lista de comentarios -->
      <div class="space-y-6 mb-8">
        <div v-for="comment in blog.comments" :key="comment.id" class="flex gap-4 group">
        ...
        <div v-if="!blog.comments?.length" class="text-center text-slate-500 py-6">Aún no hay comentarios.</div>
      </div>
      ...
      <div v-if="authStore.token && replyingTo === null" class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex gap-4">
        ...
        <div class="flex-1">
          <textarea v-model="commentContent" rows="3" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm resize-none bg-slate-50 focus:bg-white transition" placeholder="Agrega un comentario a este artículo..."></textarea>
